...
import abc
import six
from common.format_helper import timeFormat

DEFAULT_FMT = '[%(levelname)s] [%(asctime)s] %(filename)s [line:%(lineno)d]: %(message)s'
DEFAULT_DATE_FMT = '%Y-%m-%d %a, %p %H:%M:%S'
DEFAULT_LEVEL = 'DEBUG'
DEFAULT_FILENAME = './log/%Y%m%d.log'


def get_logger(loggername='', cmdlog=True, filelog=True, filename=DEFAULT_FILENAME, filemode='a', colorful=True,
...
class Logger():
    ...
    def __path_preprocess(self):
        # calculate path according to the location of log_helper.py
        self.filename = timeFormat(self.filename)
